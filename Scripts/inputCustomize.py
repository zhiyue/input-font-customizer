...
Options:
    ...
    --preset=<style>        Use a predefined style preset. Available presets:
                            andale, anonymous, consolas, dejavu, envy, fira,
                            liberation, monaco, pragmata, sourcecode
    ...
    Example 3:
    $ python /path/to/InputCustomize.py Input-Regular.ttf --preset=consolas --suffix=Custom
"""
    ...
    for argument in arguments:
        ...
        if value is None:
            ...
            if preset in style_presets:
                swap.update(style_presets[preset])
        elif key == 'help':
            print(doc)
            go = False
        else:
